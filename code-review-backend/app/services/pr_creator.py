...
class PRCreator:
    ...
    async def create_review_pr_with_changes(
        ...
        try:
            ...
            for change in approved_changes:
                ...
                # Create clean, professional commit message (no AI commentary)
                commit_msg = f"fix: Update {file_path}\n\nApply code review suggestions"
    ...
    def _format_pr_body(self, approved_changes: List[Dict]) -> str:
        """Format approved changes as clean PR description"""
        ...
        parts.append(f"- **Changes:** Code quality and security improvements\n")
        
        parts.append("\n## Files Modified\n")
        for idx, change in enumerate(approved_changes, 1):
            file_path = change['file']
            parts.append(f"{idx}. `{file_path}`")
